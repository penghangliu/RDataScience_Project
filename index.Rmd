...
**2. Data Visualizaion:** Visualizing the distribution of dependent variables(Crime Rates, including all crimes, violent crimes, firearm crimes, and property crimes) and independent variables(Gini Index, Education attainment, Unemployment Rate) over 62 counties in NY State.

**3. Generalized Additive Models:** Building Multiple Regression Model by setting Crime rates as dependent variables and Gini Index, education attainment, unemployment rate as independent variables. The output will provide coefficients of the independent varables, which provide knowledge for evluating the influence of these fators on Crimes.
...
```{r}
summary(Violent)$p.table
```

##Firearm Crimes model
```{r}
summary(Firearm)$p.table
```

##Property Crimes model
```{r}
summary(Property)$p.table
```

##Comparing 4 models
```{r, message=F, warning=F}
datatable(AIC(All,Violent,Firearm,Property))
```
